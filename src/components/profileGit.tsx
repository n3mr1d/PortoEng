"use client";
import { Github, Star, GitFork, Users, BookOpen, MapPin, Link as LinkIcon, Calendar, Code, TrendingUp, Hand } from "lucide-react";
  ...
  return (
    <div className="reveal-section  min-h-screen bg-black text-white">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-1/2 -left-1/2 w-full h-full bg-gradient-radial from-white/5 to-transparent blur-3xl"></div>
        <div className="absolute -bottom-1/2 -right-1/2 w-full h-full bg-gradient-radial from-white/5 to-transparent blur-3xl"></div>
      </div>
                ...
                <div className="w-full mt-8 space-y-3 text-left">
                  ...
                  {viewer.websiteUrl && (
                    <div className="flex items-center gap-3 text-sm text-gray-400">
                      ...
                      <a href={viewer.websiteUrl} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors">
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
              <div className="flex items-center justify-between flex-wrap gap-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-2xl font-bold">{viewer.starredRepositories.totalCount}</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-2xl font-bold">{repositories.reduce((acc, repo) => acc + repo.forkCount, 0)}</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-2xl font-bold">{repositories.reduce((acc, repo) => acc + repo.stargazerCount, 0)}</p>
...
function StatCard({ icon, label, value, trend }: { icon: React.ReactNode; label: string; value: number; trend: string }) {
  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-6 backdrop-blur-sm hover:bg-white/10 transition-all duration-300 group">
...
function RepositoryCard({ repo }: { repo: Repository }) {
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="bg-white/5 border flex flex-col block justify-center border-white/10 rounded-xl p-6 backdrop-blur-sm hover:bg-white/10 hover:border-white/20 transition-all duration-300 group"
    >
      <div className="flex items-start justify-between mb-3">
        <h4 className="font-semibold text-lg group-hover:text-white transition-colors truncate">{repo.name}</h4>
        ...
        {repo.forkCount > 0 && (
          <div className="flex items-center gap-1">
            ...
            <span>{repo.forkCount}</span>
             ...
             <div className="flex mt-3 w-full items-center gap-1">
          <Button className="text-white bg-black/50 border-2 border-gray-600" variant={"outline"}> <Hand/> Contributions Now </Button>
                </div>
